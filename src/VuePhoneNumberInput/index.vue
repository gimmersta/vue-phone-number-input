...
export default {
  ...
  computed: {
    ...
    countryCode: {
      get() {
        return this.getValidCountryCode(
          this.userLocale || this.results.countryCode
        );
      },
      set(newCountry) {
        let code = this.getValidCountryCode(newCountry);
        this.setLocale(code);
  ...
  methods: {
    getValidCountryCode(newCode) {
      ...
      try {
        ...
        let _ = getCountryCallingCode(code);
      } catch (e) {
        switch (code) {
          case "IC":
            code = "ES";
